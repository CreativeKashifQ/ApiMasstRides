    ...
    <div class="">
		...
		<div class="main-body">
			<div class="row">
				...
				<div class="col-lg-8">
					<div class="card">
                        <div class="card-header bg-green text-white">Vehicle Requests Assign to Franchise {{$franchise->name}}</div>
                        <div class="card-body">
                            <table class="table   table-striped table-sm   " >
                                ...
                                <tbody id="data">
                                    @if(isset($v_requests) && $v_requests->count() > 0)
                                    @foreach($v_requests as $key=> $v_request)
                                    <tr style="font-size: 13px;"  >
                                        <td>{{++$key}}</td>
                                        <td>{{$v_request->name}}</td>
                                        <td>{{$v_request->email}}</td>
                                        <td>{{$v_request->phone}}</td>
                                        <td>{{$v_request->country}}</td>
                                        <td>{{$v_request->state}}</td>
                                        <td>{{$v_request->city}}</td>
                                        <td>{{Carbon\Carbon::parse($v_request->created_at)->format('d M, Y')}}</td>
                                    ...
                                    @else
                                    <tr>
                                        <td colspan="9" class="text-center"><strong>No, Requests Found</strong></td>
                                    </tr>
                                    @endif
